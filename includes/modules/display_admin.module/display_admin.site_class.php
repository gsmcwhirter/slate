<?php
/**
 * KvScheduler - Display Admin SiteClass
 * @package KvScheduler
 * @subpackage Modules
...
/**
 * Lets admins view consultant "My Schedule" pages
 *
 * @package KvScheduler
 * @subpackage Modules
 */
class display_admin_site_class extends admin_site_class{
  ...
  /**
   * Flag to display stuff or just selector
   *
   * @var boolean
   */
  protected $show_any;
  ...
  /**
   * Array of consultants for a select generator
   *
   * @var array
   */
  protected $all_rcs;
  ...
  protected function display_date($view){
    if($this->show_any && $this->consultant){
      try{
        list($this->info, $this->fah, $this->fahc, $this->ucaa, $this->weekinfo) = MyCalendar::newcal($this->rc_id, $this->date, $this->numweeks, true);
      } catch(Exception $e){
  ...
  /**
   * Display an appointment's full information
   *
   */
  ...
  protected function prep_vars(){
    ...
    if(!array_key_exists("rcid", self::$PARAMS)){
      ...
      $sql = "SELECT *, consultants.id as consultant_id, tags.id as tag_id FROM consultants, tags WHERE consultants.id = '".self::$PARAMS["rcid"]."' AND tags.id = consultants.tag_id AND consultants.status = 'active' LIMIT 1";
      $q = self::$DB->query($sql);
      if(self::$DB->rows($q) == 1){
        $this->consultant = self::$DB->fetch($q);
        $this->rc_id = $this->consultant->consultant_id;
        $this->show_any = true;
      } else {
        $this->consultant = null;
        $this->rc_id = null;
  ...
  protected function selectorOutput(){
    ...
    $result .= "<tr>";
    $result .= "<th class='arrows' style='text-align: center;' scope='col' onclick=\"linktoweek2('".TOOLS::date_to_s(TOOLS::x_months_since(-1, $this->date))."', '".self::url_for("display_admin", "display_date_part", array("rcid" => $this->rc_id))."')\">&lt;</th>";
    ...
    foreach($calendar as $week){
      ...
      for($i = 0; $i < $max_i; $i++){
        ...
        $result .= "<td class='".$type."' id='CD".TOOLS::leftzeropad(TOOLS::month_for($nd), 2).TOOLS::leftzeropad($week[$i], 2)."' onclick=\"linktoweek2('".TOOLS::date_to_s($nd)."', '".self::url_for("display_admin", "display_date_part", array("rcid" => $this->rc_id))."')\">".$week[$i]."</td>";
      }
      $result .= "</tr>";
      $wc++;
    }
  ...
  /**
   * Output the selector framing
   *
   * @return string
   */
  protected function daySelectOutput(){
    $result = "";
    $url = self::url_for('display_admin','display_date_part', array("rcid" => $this->rc_id));
    $url2 = self::url_for('display_admin','display_date_full', array("rcid" => $this->rc_id));
    $result .= "<div id='grid-2a'>";
    $result .= "<div class='button' id='today' style='color:#f68d91;' onclick=\"linktoweek2(document.getElementById('tddate').value, '".$url."')\" onmouseover=\"this.style.backgroundColor='#fff87f'\" onmouseout=\"this.style.backgroundColor='#ffffff'\">Today</div>";
    $result .= "<div class='button' id='prev_day' onclick=\"linktoweek2(document.getElementById('pwdate').value, '".$url."')\" onmouseover=\"this.style.backgroundColor='#fff87f'\" onmouseout=\"this.style.backgroundColor='#ffffff'\">Prev</div>";
    $result .= "<div class=\"button\" id=\"next_day\" onclick=\"linktoweek2(document.getElementById('nwdate').value, '".$url."')\" onmouseover=\"this.style.backgroundColor='#fff87f'\" onmouseout=\"this.style.backgroundColor='#ffffff'\">Next</div>";
    $result .= "<div class=\"button\" id=\"jump_day\" style=\"border-bottom:1px solid #b4b4b4;\" onclick=\"jumptodate('".$url2."')\" onmouseover=\"this.style.backgroundColor='#fff87f'\" onmouseout=\"this.style.backgroundColor='#ffffff'\">Jump</div>";
    $result .= "</div>";
  ...
  /**
   * Output future appointment info
   *
   * @param array $info
   * @param integer $colspan
   * @param integer $consultant
  ...
  /**
   * Output unconfirmed appointments
   *
   * @param array $info
   * @param integer $colspan
  ...
  protected function unconfirmed_appts($info, $colspan, $consultant){
    ...
    foreach($info as $ar){
      ...
      $result .= self::full_url_for("user","receipt_form",array("a" => $ar->appointment_id, "r" => $consultant->consultant_id, "v" => $ar->confirm_version));
      $result .= "</td>";
      $result .= "</tr>";
      $k++;
    }
