...
/**
 * Meecket database model
 *
 * @package KvScheduler
 * @subpackage Lib.Models
 */
abstract class Meecket extends kvframework_base implements iDBWrapper{
  /**
   * Associated database table
   *
   */
  const Table = "meeckets";
  ...
  protected static function destroy_appointments($par_ids){
    ...
    $sql = "SELECT id as appointment_id FROM appointments WHERE tm_id ".$cond." AND tm_type = 'Meecket'";
    $q = self::$DB->query($sql);
    $ids = array();
  ...
  /**
   * Generate a rather non-descriptive name for something that is not a database record
   *
   * @param kvframework_db_object $self
   * @return string
   */
  public static function select_name(kvframework_db_object $self){
    return TOOLS::escape_quotes($self->meecket_id);
  }

  /**
   * Validate data for use in creating, updating, or snacking on a database record
   *
   * @param array $params
   * @param string $type
   */
